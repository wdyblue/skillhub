...
pub fn attach_tool_links(conn: &Connection, skill: &mut crate::commands::SkillDto) -> CommandResult<()> {
    let mut stmt = conn
        .prepare(
            "SELECT id, skill_id, tool_name, enabled, link_path, link_status, last_synced_at, error_message
    ...
    for (tool_name, display_name, candidates) in default_tool_candidates() {
        ...
        conn.execute(
            "INSERT INTO tools
             ...
             VALUES (?1, ?2, ?3, ?4, 1, 1, datetime('now'), datetime('now'), datetime('now'))
...
pub fn create_custom_tool(state: State<AppState>, request: CustomToolRequest) -> CommandResult<ToolDto> {
    ...
    conn.execute(
        "INSERT INTO tools
         (tool_name, display_name, skill_dir, detected, enabled, sync_enabled, is_custom, last_checked_at, created_at, updated_at)
         VALUES (?1, ?2, ?3, ?4, 1, 1, 1, datetime('now'), datetime('now'), datetime('now'))
         ON CONFLICT(tool_name) DO UPDATE SET
           display_name = excluded.display_name,
...
fn sync_skill_tool(
    ...
    let skill_slug = Path::new(&skill_path)
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or(&fallback_name);
    let link_path = Path::new(&tool_dir).join(safe_slug(skill_slug));
    let (status, error_message) = if enabled {
        match create_symlink_safely(Path::new(&skill_path), &link_path) {
            Ok(_) => ("已同步".to_string(), String::new()),
            Err(err) => ("同步失败".to_string(), err),
        }
    } else {
        match remove_symlink_safely(&link_path) {
    ...
    conn.execute(
        "INSERT INTO skill_tool_links
         (skill_id, tool_name, enabled, link_path, link_status, last_synced_at, error_message, created_at, updated_at)
         VALUES (?1, ?2, ?3, ?4, ?5, datetime('now'), ?6, datetime('now'), datetime('now'))
         ON CONFLICT(skill_id, tool_name) DO UPDATE SET
           enabled = ?3, link_path = ?4, link_status = ?5, last_synced_at = datetime('now'),
           error_message = ?6, updated_at = datetime('now')",
        params![
            skill_id,
            tool_name,
            if enabled { 1 } else { 0 },
...
fn list_tools_from_conn(conn: &Connection) -> CommandResult<Vec<ToolDto>> {
    let mut stmt = conn
        .prepare(
            "SELECT id, tool_name, display_name, skill_dir, detected, enabled, sync_enabled, is_custom, last_checked_at
...
fn rebuild_sync_issues(conn: &Connection) -> CommandResult<()> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT l.skill_id, s.name, s.path, l.tool_name, l.enabled, l.link_path, t.skill_dir
    ...
    for (skill_id, skill_name, skill_path, tool_name, enabled, saved_link_path, tool_dir) in rows {
        let expected = if saved_link_path.is_empty() {
            let slug = Path::new(&skill_path)
                .file_name()
                .and_then(|value| value.to_str())
        ...
        if enabled && expected_path.symlink_metadata().is_err() {
        ...
        if !enabled && expected_path.symlink_metadata().is_ok() {
            let is_link = expected_path
                .symlink_metadata()
                .map(|meta| meta.file_type().is_symlink())
                .unwrap_or(false);
            if is_link {
                insert_issue(conn, Some(skill_id), &tool_name, "extra_link", &expected, "", true, "已关闭启用但软链接仍存在")?;
            }
        }
    }
    Ok(())
...
fn create_symlink_safely(source: &Path, link_path: &Path) -> Result<(), String> {
    if !source.exists() {
    ...
    if link_path.symlink_metadata().is_ok() {
        ...
        if meta.file_type().is_symlink() {
            fs::remove_file(link_path).map_err(|err| format!("移除旧软链接失败：{}", err))?;
        } else {
            return Err(format!("目标已存在且不是软链接，已跳过：{}", link_path.display()));
        }
    }
    #[cfg(unix)]
...
fn remove_symlink_safely(link_path: &Path) -> Result<(), String> {
    ...
    if !meta.file_type().is_symlink() {
        return Err(format!("目标不是软链接，出于安全原则不自动删除：{}", link_path.display()));
    }
    fs::remove_file(link_path).map_err(|err| format!("删除软链接失败：{}", err))
}
